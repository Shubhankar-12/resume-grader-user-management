...
export class PaymentSubscriptionQueries {
  ...
  async getPaymentSubscriptionbyUserId(data: any): Promise<any[]> {
    ...
    aggregateQuery.push({ $match: { user_id: new ObjectId(data.user_id) } });
    if (data.status) {
      aggregateQuery.push({ $match: { status: data.status } });
    }
  ...
  async getPaymentSubscriptionbyId(data: any): Promise<any[]> {
    ...
    aggregateQuery.push({ $match: { _id: new ObjectId(data.payment_subscription_id) } });
  ...
  async updatePaymentSubscription(data: any): Promise<any> {
    ...
    return await this.paymentSubscriptionModel.updateOne(filter, { $set: data });
  }
